...
enum ControllerCLI {
    static let usage = """
    owndesk-controller-cli <command> [options]
    ...
    static func main() async throws {
        ...
        print("owndesk-controller-cli on \(config.deviceName): identity \(identity.fingerprint)")
    ...
    /// `owndesk-controller-cli app …`: drives the running OwnDesk app through its local control channel.
    static func controlApp(_ argsIn: [String]) async {
        var args = argsIn
        // OwnDesk.app keeps its data, and so its control file, beside this CLI's own folder.
        var dataDir = ControllerConfig.standard().dataDirectory.deletingLastPathComponent().appendingPathComponent("OwnDesk", isDirectory: true)
        ...
        do {
            ...
            print("OwnDesk is not running (no control.json in \(dataDir.path))")
            exit(3)
        } catch {
