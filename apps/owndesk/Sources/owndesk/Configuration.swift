...
/// One app, one identity, one folder. The two halves of OwnDesk used to keep their own of each, which
...
struct AppConfiguration {
    ...
    static func fromCommandLine(_ arguments: [String] = Array(CommandLine.arguments.dropFirst())) -> AppConfiguration {
        ...
        let standardDirectory = support.appendingPathComponent("OwnDesk", isDirectory: true)
        if !arguments.contains("--data-dir") { adoptPRCInstall(support: support, into: standardDirectory) }
        var config = AppConfiguration(
            dataDirectory: standardDirectory,
    ...
    /// OwnDesk was called PRC until September 2026. The first launch under the new name takes over
    /// the old data folder, which holds the identity and the pairings, so no device has to pair
    /// again, and the old settings, so hosting stays as the user left it.
    static func adoptPRCInstall(support: URL, into directory: URL) {
        let files = FileManager.default
        let legacy = support.appendingPathComponent("PRC", isDirectory: true)
        if !files.fileExists(atPath: directory.path), files.fileExists(atPath: legacy.path) {
            try? files.moveItem(at: legacy, to: directory)
        }
        let defaults = UserDefaults.standard
        if let bundleId = Bundle.main.bundleIdentifier, defaults.persistentDomain(forName: bundleId)?.isEmpty ?? true,
           let old = defaults.persistentDomain(forName: "com.prc.app"), !old.isEmpty {
    ...
    func loadIdentity() throws -> any SigningIdentity {
        ...
        return try IdentityStore.loadOrCreate(service: "owndesk.identity")
    }
}
