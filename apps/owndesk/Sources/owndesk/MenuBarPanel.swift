...
struct MenuBarPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack {
                Button("Open OwnDesk…") {
                    if !AppDelegate.showExistingWindow() { openWindow(id: OwnDeskApp.windowID) }
                }
                Spacer()
        ...
        .padding(14)
        .frame(width: 320)
        .onReceive(NotificationCenter.default.publisher(for: .owndeskOpenWindow)) { _ in
            openWindow(id: OwnDeskApp.windowID)
        }
    }
    ...
    @ViewBuilder private var permissions: some View {
        ...
        if !state.screenRecording || !state.accessibility {
            Text("If the switch in System Settings is already on, it belongs to a previous build: remove OwnDesk from that list, grant again when asked, then quit and reopen.")
