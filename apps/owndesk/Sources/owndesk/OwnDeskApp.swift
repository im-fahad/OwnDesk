...
struct OwnDeskApp: App {
    ...
    var body: some Scene {
        Window("OwnDesk", id: OwnDeskApp.windowID) {
            ContentView()
    ...
    static let windowID = "owndesk.main"
}

extension Notification.Name {
    static let owndeskOpenWindow = Notification.Name("owndesk.openWindow")
    /// Posted when the Dock icon appears or disappears, which costs the video view its drawing.
    static let owndeskRebuildVideo = Notification.Name("owndesk.rebuildVideo")
}
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    static func showDockIcon(_ visible: Bool) {
        ...
        NotificationCenter.default.post(name: .owndeskRebuildVideo, object: nil)
    }
    ...
    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        if !flag, !AppDelegate.showExistingWindow() {
            NotificationCenter.default.post(name: .owndeskOpenWindow, object: nil)
        }
        return true
    }
