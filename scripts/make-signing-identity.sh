...
openssl pkcs12 -export -inkey "$TMP/key.pem" -in "$TMP/cert.pem" -out "$TMP/identity.p12" -passout pass:owndesk -name "$NAME"

KEYCHAIN="$HOME/Library/Keychains/login.keychain-db"
security import "$TMP/identity.p12" -k "$KEYCHAIN" -P owndesk -T /usr/bin/codesign -T /usr/bin/security
...
echo "created identity \"$NAME\". Build with:  OWNDESK_SIGN_IDENTITY=\"$NAME\" scripts/build-apps.sh"
